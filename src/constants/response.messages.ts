export const JobSeekerProfileMessages = {
  // Success messages
  CREATE_SUCCESS: 'Job seeker profile created successfully.',
  GET_SUCCESS: 'Job seeker profile retrieved successfully.',
  UPDATE_SUCCESS: 'Job seeker profile updated successfully.',

  // Error messages
  CREATE_FAILURE: 'Failed to create job seeker profile.',
  GET_FAILURE: 'Failed to retrieve job seeker profile.',
  UPDATE_FAILURE: 'Failed to update job seeker profile.',
};

export const FILE_UPLOAD_MESSAGES = {
  NO_FILE_UPLOADED: 'No file uploaded.',
  FILE_NOT_FOUND: 'File not found.',
  RESUME_REQUIRED: 'Resume file is required.',
  LOGO_REQUIRED: 'Logo file is required.',
  COVER_LETTER_REQUIRED: 'Cover letter file is required.',
  RESUME_UPLOAD_SUCCESS: 'Resume uploaded successfully.',
  UPLOAD_FAILED: 'Cloudinary upload failed.',
  UNSUPPORTED_FILE_TYPE: 'Unsupported file type.',
  FILE_TOO_LARGE: 'File exceeds size limit.',
  UNSUPPORTED_FILE_EXTENSION: 'Unsupported file extension.',
};
