...
const ensureValidFile = (file: Express.Multer.File | undefined, rules?: FileValidationOptions) => {
  ...
  if (rules.required && !file) throw new AppError(msg.required, StatusCodes.BAD_REQUEST || 400);
  if (!file) return;

  if (rules.maxBytes && file.size > rules.maxBytes) {
    throw new AppError(msg.size, StatusCodes.BAD_REQUEST || 400);
  }

  if (rules.allowedMimeTypes?.length && !rules.allowedMimeTypes.includes(file.mimetype)) {
    throw new AppError(msg.type, StatusCodes.BAD_REQUEST || 400);
  }

  const ext = (file.originalname.split('.').pop() ?? '').toLowerCase();
  const allowedExt = rules.allowedExtensions?.map((e) => e.toLowerCase());
  if (allowedExt?.length && (!ext || !allowedExt.includes(ext))) {
    throw new AppError(msg.extension, StatusCodes.BAD_REQUEST || 400);
  }
};
  ...
  return (req, res, next) => {
    single(req, res, (err) => {
      if (err) {
        ...
        return next(new AppError(message, StatusCodes.BAD_REQUEST || 400, true, err as Error));
      }
